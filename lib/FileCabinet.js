 ...
 * @param callback
 */
module.exports.getFile = function (internalid, destinationPath, configPath, passphrase, callback) {
    init(configPath, passphrase);
    ...
    get("file", internalid, function (err, response, xmlbody) {
        ...
        xmlparser.parseString(xmlbody, function (err, result) {
            if (!err) {
                ...
                try {
                    var filePath = path.resolve(destinationPath, record.name);
                    var doWriteFile = true;
                    if (fs.existsSync(filePath)) {
                        doWriteFile = readlineSync.keyInYN('File ' + record.name + ' already exists. Overwrite?');
                    }
                    if (doWriteFile) {
                        fs.writeFileSync(filePath, record.content, 'base64');
                    }
                } catch (e) {
                    console.log(e);
                    if (callback)  return callback(false);
